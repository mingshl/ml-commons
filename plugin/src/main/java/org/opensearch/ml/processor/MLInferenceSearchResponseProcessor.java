...
public class MLInferenceSearchResponseProcessor extends AbstractProcessor implements SearchResponseProcessor, ModelExecutor {
    ...
    private ActionListener<Map<Integer, MLOutput>> createRewriteResponseListener(
        SearchResponse response,
        ActionListener<SearchResponse> responseListener,
        java.util.List<java.util.Map<String, String>> processInputMap,
        java.util.List<java.util.Map<String, String>> processOutputMap,
        java.util.Map<Integer, Integer> hitCountInPredictions,
        String queryString
    ) {
    ...
    private boolean checkIsModelInputMissing(Map<String, Object> document, String queryString, Map<String, String> inputMapping) {
        ...
        for (Map.Entry<String, String> inputMapEntry : inputMapping.entrySet()) {
            ...
            boolean checkSingleModelInputPresentInDocument = false;
            if (document != null) {
                checkSingleModelInputPresentInDocument = hasField(document, oldDocumentFieldName);
            }
            boolean checkSingleModelInputPresentInQuery = false;
            if (queryString != null) {
                checkSingleModelInputPresentInQuery = hasField(queryString, oldDocumentFieldName);
            }
            if (!checkSingleModelInputPresentInDocument && !checkSingleModelInputPresentInQuery) {
