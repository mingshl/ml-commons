...
// Note: ML_CONTEXT_MANAGEMENT_INDEX constant needs to be added to CommonValue
...
public class ContextManagementTemplateService {
    ...
    @Inject
    public ContextManagementTemplateService(
                ...
                client.get(getRequest, ActionListener.wrap(
                    getResponse -> {
                        if (!getResponse.isExists()) {
                            wrappedListener.onFailure(
                                new MLResourceNotFoundException("Context management template not found: " + templateName)
                            );
                            return;
                        }
    ...
    private void validateUniqueTemplateName(String templateName, ActionListener<Boolean> listener) {
        try {
            SearchRequest searchRequest = new SearchRequest(ContextManagementIndexUtils.getIndexName());
            SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder()
                .query(new TermQueryBuilder("_id", templateName))
                .size(1);
            searchRequest.source(searchSourceBuilder);
    ...
    private XContentParser createXContentParserFromRegistry(
        ...
        return MediaTypeRegistry.JSON.xContent()
            .createParser(xContentRegistry, deprecationHandler, bytesReference.streamInput());
    }
}