...
public class MLPlanExecuteAndReflectAgentRunner implements MLAgentRunner {
    ...
    private void executePlanningLoop(
        ...
        if (hookRegistry != null && !completedSteps.isEmpty()) {
            ...
            try {
                ContextManagerContext context = AgentContextUtil.emitPreLLMHook(requestParams, completedSteps, null, memory, hookRegistry);
                if (context != null) {
                    AgentContextUtil.updateParametersFromContext(requestParams, context);
                }
            } catch (Exception e) {
