...
public class MLChatAgentRunner implements MLAgentRunner {
    ...
    private void runReAct(
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            lastStepListener.whenComplete(output -> {
                ...
                if (finalI % 2 == 0) {
                    ...
                    AgentContextUtil.emitPreLLMHook(tmpParameters, interactions, currentToolSpecs, memory, hookRegistry);
        ...
        if(hookRegistry!=null) {
            AgentContextUtil.emitPreLLMHook(tmpParameters, interactions, initialToolSpecs, memory, hookRegistry);
        }
        ActionRequest request = streamingWrapper.createPredictionRequest(llm, tmpParameters, tenantId);
