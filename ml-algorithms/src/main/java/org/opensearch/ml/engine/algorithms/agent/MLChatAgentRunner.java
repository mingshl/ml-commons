...
public class MLChatAgentRunner implements MLAgentRunner {
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            lastStepListener.whenComplete(output -> {
                ...
                if (finalI % 2 == 0) {
                    ...
                    // Emit POST_TOOL hook event after tool execution
                    List<MLToolSpec> postToolSpecs = new ArrayList<>(toolSpecMap.values());
                    emitPostToolHook(tmpParameters, interactions, postToolSpecs, memory);
    ...
    /**
     * Emit POST_TOOL hook event
     */
    private void emitPostToolHook(Map<String, String> parameters, List<String> interactions, List<MLToolSpec> toolSpecs, Memory memory) {
        if (hookRegistry != null) {
            try {
                ContextManagerContext context = buildContextManagerContext(parameters, interactions, toolSpecs, memory);
                // Create tool results from interactions
                List<Map<String, Object>> toolResults = new ArrayList<>();
                for (String interaction : interactions) {
                    Map<String, Object> toolResult = new HashMap<>();
                    toolResult.put("output", interaction);
                    toolResults.add(toolResult);
                }
                EnhancedPostToolEvent event = new EnhancedPostToolEvent(toolResults, null, context, new HashMap<>());
                hookRegistry.emit(event);
                log.debug("Emitted POST_TOOL hook event");
            } catch (Exception e) {
